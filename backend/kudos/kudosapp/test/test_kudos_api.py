...
def test_give_kudos_success(auth_client, receiver, user):
    ...
    response = auth_client.post(url, {"receiver": receiver.email, "message": "You're amazing!"})
    assert response.status_code == status.HTTP_201_CREATED or status.HTTP_200_OK
...
def test_give_kudos_invalid_receiver(auth_client):
    url = reverse("kudos-create")
    response = auth_client.post(url, {"receiver": "invalid_email", "message": "You're amazing!"})
    assert response.status_code == status.HTTP_400_BAD_REQUEST
...
def test_give_kudos_to_sender(auth_client, user):
    url = reverse("kudos-create")
    response = auth_client.post(url, {"receiver": user.email, "message": "You're amazing!"})
    assert response.status_code == status.HTTP_400_BAD_REQUEST
...
def test_kudos_limit(auth_client, receiver, user):
    ...
    Kudos.objects.bulk_create([
        Kudos(sender=user, receiver=receiver, message="Test", timestamp=now - timedelta(days=7))
        for _ in range(3)
    ])
    url = reverse("kudos-create")
    response = auth_client.post(url, {"receiver": receiver.email, "message": "One more"})
    assert response.status_code == status.HTTP_400_BAD_REQUEST
